...
const fs = require('fs');
const os = require('os');
// const ftp = require('basic-ftp'); // FTP 사용 안함
...
// 로컬 파일 경로 설정
const localFilePath = path.join(__dirname, '장비솔루션_파트재고.xlsx');
...
// 로컬 파일을 임시 파일로 복사하는 함수
async function downloadFileFromFTP() {
  try {
    console.log('로컬 파일 복사 중...');
    
    // 원본 파일이 존재하는지 확인
    if (!fs.existsSync(localFilePath)) {
      throw new Error(`원본 파일이 존재하지 않습니다: ${localFilePath}`);
    }
    
    // 파일 복사
    await fse.copy(localFilePath, localTempFile);
    console.log(`파일 복사 완료: ${localTempFile}`);
...
// 임시 파일을 로컬 파일로 저장하는 함수
async function uploadFileToFTP() {
  try {
    console.log('로컬 파일 저장 중...');
    
    // 파일 복사 (임시 파일 -> 원본 파일)
    await fse.copy(localTempFile, localFilePath);
    console.log(`파일 저장 완료: ${localFilePath}`);
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    console.log('초기 파일 복사 완료');
  } catch (error) {
    console.error('초기 파일 복사 실패:', error);
  }
...
// 부품 등록 처리 핸들러 수정
ipcMain.handle('register-part', async (event, partData) => {
    try {
...
// 재고 업데이트 처리 핸들러
ipcMain.handle('update-stock', async (event, data) => {
  try {
    ...
    // 로그 기록
    console.log(`재고 업데이트 성공: ${data.name}, ${data.action}, ${data.amount}개, 새 재고: ${data.currentStock}, 카테고리: ${partCategory}`);
